...
void require(bool ok,const char* why) {
    if(!ok){std::fprintf(stderr,"FAIL: %s (calls=%s)\n",why,calls.c_str());std::exit(1);}
}
void attributes(){calls+='A';SDL_GL_SetAttribute(SDL_GL_DEPTH_SIZE,24);}
bool begin(GBContext*,bool,const uint32_t*) {calls+='B';return predict;}
...
bool event(const SDL_Event* e,bool) {
    if(e->type==SDL_KEYDOWN&&e->key.keysym.scancode==SDL_SCANCODE_F2){calls+='E';++consumed;return true;}
    return false;
}
void capture(int w,int h,bool) {
    calls+='C';captured.resize(size_t(w)*h*4);glReadPixels(0,0,w,h,GL_RGBA,GL_UNSIGNED_BYTE,captured.data());
    require(glGetError()==GL_NO_ERROR,"capture runs with a current GL context");
}
void shutdown(){calls+='S';require(SDL_GL_GetCurrentContext()!=nullptr,"shutdown before GL destruction");}
...
std::vector<uint8_t> surface() {
    GLint vp[4];glGetIntegerv(GL_VIEWPORT,vp);std::vector<uint8_t> out(size_t(vp[2])*vp[3]*4);
    glReadPixels(0,0,vp[2],vp[3],GL_RGBA,GL_UNSIGNED_BYTE,out.data());return out;
}
}
int main(int argc,char** argv) {
    ...
    require(varied>baseline.size()/16,"ordinary LCD is visibly rendered, not an empty comparison");
    gb_platform_shutdown();
    GBPresentationHooks hooks{};hooks.api_version=GB_PRESENTATION_API_VERSION;hooks.struct_size=sizeof(hooks);
    hooks.gl_attributes=attributes;hooks.begin_frame=begin;hooks.frame=frame;hooks.event=event;
    hooks.before_swap=capture;hooks.shutdown=shutdown;hooks.input_poll=poll;hooks.state_loaded=state_loaded;
    require(gb_platform_set_presentation(&hooks),"register matching version");
    auto bad=hooks;++bad.api_version;require(!gb_platform_set_presentation(&bad),"wrong version rejected");
    ...
    external=0xfe;require(gb_platform_poll_events(ctx)&&g_joypad_dpad==0xfe,"external right input");
    gb_platform_set_input_script("c0:U:100000");require(gb_platform_poll_events(ctx)&&g_joypad_dpad==0xfa,"independent script and external channels");
    external=255;gb_platform_set_input_script(nullptr);require(gb_platform_poll_events(ctx)&&g_joypad_dpad==255,"both channels release");
    key.key.keysym.scancode=SDL_SCANCODE_W;key.key.keysym.sym=SDLK_w;
    require(SDL_PushEvent(&key)==1&&gb_platform_poll_events(ctx),"manual key press");
    require((g_joypad_dpad&4)==0,"W binding presses up");
    const SDL_Scancode release[]={SDL_SCANCODE_W};gb_platform_release_keys(release,1);
    require(gb_platform_poll_events(ctx)&&g_joypad_dpad==255,"mode change releases held key bindings");
    gb_platform_set_input_record_file("external.input");ctx->cycles=100;external=0xfe;
    require(gb_platform_poll_events(ctx),"record external press");ctx->cycles=200;external=255;
    require(gb_platform_poll_events(ctx),"record external release");
    gb_platform_set_input_record_file(nullptr);
    FILE* recorded=std::fopen("external.input","rb");require(recorded,"external recording file");
    char text[512]{};std::fread(text,1,sizeof(text)-1,recorded);std::fclose(recorded);
    require(std::strstr(text,"c100:R:100"),"recording includes external channel with original cycle anchors");
    auto press=[&](SDL_Scancode scancode,SDL_Keycode symbol) {
        SDL_Event e{};e.type=SDL_KEYDOWN;e.key.keysym.scancode=scancode;e.key.keysym.sym=symbol;
        require(SDL_PushEvent(&e)==1&&gb_platform_poll_events(ctx),"savestate shortcut down");
        e.type=SDL_KEYUP;require(SDL_PushEvent(&e)==1&&gb_platform_poll_events(ctx),"savestate shortcut up");
    };
    std::remove("game.state1");press(SDL_SCANCODE_F8,SDLK_F8);require(loaded==0,"failed load does not notify presentation");
    ctx->wram[0]=0x31;press(SDL_SCANCODE_F5,SDLK_F5);ctx->wram[0]=0x62;
    press(SDL_SCANCODE_F8,SDLK_F8);require(loaded==1&&ctx->wram[0]==0x31,"successful frontend load notifies after restoring state");
    calls.clear();gb_platform_shutdown();require(calls=="S","shutdown callback runs once before destroying GL");
    require(gb_platform_set_presentation(nullptr),"unregister after shutdown");require(calls=="S","unregister does not duplicate shutdown");
    gb_context_destroy(ctx);std::puts("PASS: presentation lifecycle, version, fallback pixels, events and independent input channels");
}
